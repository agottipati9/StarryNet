...
def run_experiment(args, total_duration):
    ...
    call_offset = 10
    if args.exp == 2 or args.exp == 3:  # Add maneuvers to LEO scenarios
        # Model maneuvers as a Poisson process
        maneuver_rate = 0.01  if args.exp == 2 else np.random.uniform(0.02, 0.05) # 1/100 vs. 1/50 to 1/20 events/sec
        ...
        for t in maneuver_times:
            ratio = np.random.uniform(0.01, 0.03) if args.exp == 2 else np.random.uniform(0.05, 0.1) # 1–3% is minor vs. 5-10% is major
            sn.set_damage(ratio, int(t))  # Apply maneuver at time t
            recovery_time = np.random.randint(3, 5) if args.exp == 2 else np.random.randint(5, 10) # Quick recovery (3–5 s) vs. 5-10s for major
